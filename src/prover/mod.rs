    ...
    B: SnarkBackend,
{
    ...
    pub fn track_and_commit_mat_mv_poly(
        ...
        let result = self
            .tracker_rc
            .borrow_mut()
            .track_and_commit_mat_mv_p(polynomial, false)?;
        let tracked_poly = match result {
            Either::Left(id) => {
                TrackedPoly::new(Either::Left(id), num_vars, self.tracker_rc.clone())
            }
            Either::Right((id, cnst)) => {
                TrackedPoly::new_committed_constant(cnst, id, num_vars, self.tracker_rc.clone())
            }
        };
    ...
    pub fn track_mat_mv_poly_with_commitment(
        ...
        let tracked_poly = TrackedPoly::new(
            Either::Left(
                self.tracker_rc
                    .borrow_mut()
                    .track_mat_mv_p_with_commitment(polynomial, commitment, binding, false)?,
            ),
            num_vars,
            self.tracker_rc.clone(),
        );
    ...
    pub fn track_mat_uv_poly_with_commitment(
        ...
        let tracked_poly = TrackedPoly::new(
            Either::Left(
                self.tracker_rc
                    .borrow_mut()
                    .track_mat_uv_p_with_commitment(polynomial, commitment, binding)?,
            ),
            degree,
            self.tracker_rc.clone(),
        );
...
mod tests {
    use super::*;
    use crate::{DefaultSnarkBackend, test_utils::test_prelude};

    type F = <DefaultSnarkBackend as crate::SnarkBackend>::F;

    fn setup() -> ArgProver<DefaultSnarkBackend> {
        let (prover, _verifier) = test_prelude::<DefaultSnarkBackend>().unwrap();
        prover
    }

    // ── TrackedPoly constant folding ────────────────────────────────

    #[test]
    fn constant_plus_constant_folds() {
        let mut prover = setup();
        let a = prover.track_mat_mv_cnst_poly(3, F::from(10));
        let b = prover.track_mat_mv_cnst_poly(3, F::from(20));
        let c = &a + &b;
        assert_eq!(c.id_or_const(), Either::Right(F::from(30)));
    }

    #[test]
    fn constant_times_constant_folds() {
        let mut prover = setup();
        let a = prover.track_mat_mv_cnst_poly(3, F::from(5));
        let b = prover.track_mat_mv_cnst_poly(3, F::from(7));
        let c = &a * &b;
        assert_eq!(c.id_or_const(), Either::Right(F::from(35)));
    }

    #[test]
    fn constant_minus_constant_folds() {
        let mut prover = setup();
        let a = prover.track_mat_mv_cnst_poly(3, F::from(100));
        let b = prover.track_mat_mv_cnst_poly(3, F::from(30));
        let c = &a - &b;
        assert_eq!(c.id_or_const(), Either::Right(F::from(70)));
    }

    #[test]
    fn poly_plus_constant_stays_tracked() {
        let mut prover = setup();
        let p = prover.track_mat_mv_poly(MLE::from_evaluations_vec(
            2,
            vec![F::from(1), F::from(2), F::from(3), F::from(4)],
        ));
        let c = prover.track_mat_mv_cnst_poly(2, F::from(10));
        let result = &p + &c;
        assert!(result.id_or_const().is_left());
    }

    #[test]
    fn constant_times_poly_stays_tracked() {
        let mut prover = setup();
        let p = prover.track_mat_mv_poly(MLE::from_evaluations_vec(
            2,
            vec![F::from(1), F::from(2), F::from(3), F::from(4)],
        ));
        let c = prover.track_mat_mv_cnst_poly(2, F::from(3));
        let result = &c * &p;
        assert!(result.id_or_const().is_left());
    }

    #[test]
    fn constant_degree_is_zero() {
        let mut prover = setup();
        let c = prover.track_mat_mv_cnst_poly(3, F::from(42));
        assert_eq!(c.degree(), 0);
    }

    #[test]
    fn tracked_poly_degree_is_one() {
        let mut prover = setup();
        let p = prover.track_mat_mv_poly(MLE::from_evaluations_vec(
            2,
            vec![F::from(1), F::from(2), F::from(3), F::from(4)],
    ...
    #[test]
    fn product_of_tracked_polys_has_degree_two() {
        let mut prover = setup();
        let a = prover.track_mat_mv_poly(MLE::from_evaluations_vec(
            2,
            vec![F::from(1), F::from(2), F::from(3), F::from(4)],
        ));
        let b = prover.track_mat_mv_poly(MLE::from_evaluations_vec(
            2,
    ...
    // ── Committed constant detection via ArgProver ──────────────────

    #[test]
    fn committed_constant_detected_and_tracked() {
        let mut prover = setup();
    ...
    #[test]
    fn committed_non_constant_is_left() {
        let mut prover = setup();
        let mle =
