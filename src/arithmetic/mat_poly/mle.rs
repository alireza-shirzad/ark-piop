    ...
    fn deserialize_with_mode<R: std::io::Read>(
        ...
        match tag {
            ...
            7 => {
                ...
                let indices = Vec::<u32>::deserialize_with_mode(&mut reader, compress, validate)?;
                let values = Vec::<F>::deserialize_with_mode(&mut reader, compress, validate)?;
                let n = u64::deserialize_with_mode(&mut reader, compress, validate)? as usize;
...
impl<F: Field> MLEStorage<F> {
    ...
    pub fn lift(&self, i: usize) -> F {
        match self {
            ...
            Self::LazyInverseShiftedSum { s1, s2, shift, .. } => {
                ...
                let v1 = (s1_storage.lift(idx1) - *shift)
                    .inverse()
                    .unwrap_or_else(F::zero);
                let v2 = (s2_storage.lift(idx2) - *shift)
                    .inverse()
                    .unwrap_or_else(F::zero);
                v1 + v2
            }
    ...
    pub fn heap_bytes(&self) -> u64 {
        match self {
            ...
            Self::Rle { runs, .. } => (runs.len() as u64) * ((std::mem::size_of::<F>() as u64) + 4),
            Self::Sparse { exceptions, .. } => {
        ...
        F: ark_ff::PrimeField,
    {
        ...
        fn scan_bits<F: Field>(bits: &[u8], len: usize, max_runs: usize) -> Option<Vec<(F, u32)>> {
            if len == 0 {
                return None;
            }
        ...
        let Some(runs) = runs else {
            ...
            let out = match &self {
                Self::Field(m) => {
                    ...
                    let max_ex = (current_bytes.saturating_sub(f_bytes) / (2 * entry)) as usize;
                    if max_ex == 0 {
                        return self;
                    }
                    scan_sparse_native(&m.evaluations, max_ex)
                        .and_then(|(d, ex)| Self::new_sparse(d, ex, inner_nv))
                }
                Self::Bit { bits, .. } => {
...
impl<F: Field> MLE<F> {
    ...
    pub fn from_bits<I: IntoIterator<Item = bool>>(iter: I, num_vars: usize) -> Self {
        ...
        for (i, b) in iter.into_iter().enumerate() {
            assert!(
                i < len,
                "from_bits: iterator yields more than 2^num_vars items"
            );
            if b {
                bits[i >> 3] |= 1u8 << (i & 7);
            }
        ...
        assert_eq!(
            count, len,
            "from_bits: iterator yielded {} items, expected {}",
            count, len
        );
        Self {
            storage: MLEStorage::Bit {
    ...
    pub fn from_sparse(default: F, exceptions: Vec<(u32, F)>, num_vars: usize) -> Option<Self>
    where
        F: ark_ff::PrimeField,
    {
        Some(Self {
    ...
    /// [`Self::from_sparse`].
    pub fn from_sparse_u8(default: u8, exceptions: Vec<(u32, u8)>, num_vars: usize) -> Option<Self>
    where
        F: ark_ff::PrimeField,
    {
        Some(Self {
    ...
    pub fn from_packed_decimal(high: Vec<u64>, low: Vec<u64>, scale: u8, num_vars: usize) -> Self {
        assert!(
            high.len() == low.len(),
        ...
        F: ark_ff::PrimeField,
    {
        ...
        match kind {
            ...
            Kind::U64 => {
                let words: Vec<u64> = evals.iter().map(|x| x.into_bigint().as_ref()[0]).collect();
    ...
    pub fn evaluations(&self) -> Vec<F> {
        match (self.nv, &self.storage) {
            ...
            (Some(nv), _) => cyclic_repeat(self.storage.to_evaluations_vec(), 1usize << nv),
    ...
    pub fn iter_mut(&mut self) -> IterMut<'_, F> {
        assert!(
            self.nv.is_none(),
            "iter_mut is not supported when nv is set"
        );
        match &mut self.storage {
            MLEStorage::Field(m) => m.evaluations.iter_mut(),
            _ => {
                panic!("iter_mut called on compressed MLE storage; promote to Field storage first")
            }
        }
    }
                ...
                match self_field.num_vars.cmp(&rhs_field.num_vars) {
                    Ordering::Less => MLE {
                        storage: MLEStorage::Field(
                            &increase_nv(self_field.as_ref(), rhs_field.num_vars)
                                + rhs_field.as_ref(),
                        ),
                        nv: Some(nv1),
                    },
                    Ordering::Greater => MLE {
                        storage: MLEStorage::Field(
                            self_field.as_ref()
                                + &increase_nv(rhs_field.as_ref(), self_field.num_vars),
                        ),
                        nv: Some(nv1),
                    },
                    Ordering::Equal => MLE {
...
mod tests {
        ...
        for nv_inner in 1usize..=4 {
            ...
            for &outer_nv in &[None, Some(nv_inner + 1), Some(nv_inner + 2)] {
                ...
                assert_eq!(
                    fresh.num_vars(),
                    baseline.num_vars(),
                    "nv_inner={nv_inner} outer={outer_nv:?}"
                );
                assert_eq!(
                    fresh.evaluations(),
    ...
    fn rle_variant_lifts_by_run_boundaries() {
        ...
        let expected = vec![fr(7), fr(7), fr(7), fr(0), fr(0), fr(5), fr(5), fr(5)];
    ...
    fn detect_redundancy_bails_when_too_many_runs() {
        ...
        let out = storage.detect_redundancy();
        assert!(
            matches!(out, MLEStorage::Bit { .. }),
            "expected Bit fallback"
        );
    }

    #[test]
    fn detect_redundancy_is_noop_on_constant_and_rle() {
        let c: MLEStorage<Fr> = MLEStorage::Constant {
            value: fr(3),
            inner_num_vars: 4,
        };
        assert!(matches!(
            c.clone().detect_redundancy(),
            MLEStorage::Constant { .. }
        ));
        let r = MLEStorage::<Fr>::new_rle(vec![(fr(1), 4), (fr(0), 12)], 4).unwrap();
        assert!(matches!(
            r.clone().detect_redundancy(),
            MLEStorage::Rle { .. }
        ));
    }
    ...
    fn from_bit_backing_small_num_vars_reports_correct_num_vars() {
        for nv in 0..=2 {
            ...
            assert_eq!(
                mle.num_vars(),
                nv,
                "from_bit_backing(_, {}) must report num_vars=={}",
                nv,
                nv
            );
        }
    ...
    fn from_prefix_activator_partial_active_len_is_two_run_rle() {
        ...
        let expected: Vec<Fr> = (0..8)
            .map(|i| if i < 5 { Fr::one() } else { Fr::zero() })
            .collect();
    ...
    fn from_prefix_activator_heap_bytes_dwarfs_prior_bit_backing() {
        ...
        assert!(
            bytes < 200,
            "prefix activator storage should be tiny, got {bytes} bytes"
        );
    ...
    fn from_window_activator_large_nv_stays_tiny() {
        ...
        assert!(
            bytes < 300,
            "window activator storage should be tiny, got {bytes} bytes"
        );
    }
    ...
    fn sparse_lift_returns_default_and_exception_values() {
        ...
        let storage =
            MLEStorage::<Fr>::new_sparse(fr(0), vec![(2, fr(42)), (5, fr(99))], 3).unwrap();
        for i in 0..8 {
            let want = match i {
                2 => fr(42),
                5 => fr(99),
    ...
    fn sparse_constructor_rejects_duplicate_indices() {
        let out = MLEStorage::<Fr>::new_sparse(fr(0), vec![(2, fr(1)), (2, fr(2))], 3);
    ...
    fn sparse_constructor_rejects_out_of_range_index() {
        ...
        let out = MLEStorage::<Fr>::new_sparse(fr(0), vec![(8, fr(1))], 3);
    ...
    fn sparse_serialize_roundtrip() {
        ...
        let storage =
            MLEStorage::<Fr>::new_sparse(fr(7), vec![(1, fr(11)), (4, fr(13)), (9, fr(17))], 4)
                .unwrap();
        let mut buf: Vec<u8> = Vec::new();
        storage
            .serialize_with_mode(&mut buf, Compress::Yes)
    ...
    fn sparse_matches_field_evaluations() {
        ...
        let sparse =
            MLE::<Fr>::from_sparse(fr(3), vec![(7, fr(100)), (19, fr(200)), (31, fr(300))], nv)
                .unwrap();
    ...
    fn detect_redundancy_finds_sparse_from_field_storage() {
        ...
        let dense =
            MLEStorage::<Fr>::Field(DenseMultilinearExtension::from_evaluations_vec(nv, evals));
    ...
    fn sparse_heap_bytes_beats_dense_for_scattered_columns() {
        ...
        assert!(
            sparse_bytes < 4_000,
            "sparse should be ~3.6 KB, got {sparse_bytes}"
        );
    ...
    fn sparse_u8_lift_matches_dense_u8_field_evaluations() {
        ...
        dense[11] = 200;
        dense[20] = 250;
        let mle = MLE::<Fr>::from_sparse_u8(7u8, vec![(3, 100), (11, 200), (20, 250)], nv).unwrap();
    ...
    fn sparse_u64_lift_matches_dense() {
        ...
        dense[7] = u64::MAX - 1;
        let mle =
            MLE::<Fr>::from_sparse_u64(u64::MAX, vec![(0, 0), (7, u64::MAX - 1)], nv).unwrap();
    ...
    fn sparse_u8_serialize_roundtrip() {
        ...
        let storage = MLEStorage::<Fr>::new_sparse_u8(5u8, vec![(1, 100), (3, 200)], 3).unwrap();
        let mut buf: Vec<u8> = Vec::new();
        storage
            .serialize_with_mode(&mut buf, Compress::Yes)
            .unwrap();
