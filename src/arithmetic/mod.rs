//! Arithmetic primitives: field utilities, polynomial representations, and
//! virtual (symbolic sum-of-products) polynomials.
...
pub(crate) fn f_short_str<F: Field>(f: F) -> String {
    let f_str = format!("{:?}", f);
    if f_str.len() <= 4 {
        f_str
    } else {
...
pub(crate) fn f_vec_short_str<F: Field>(f: &[F]) -> String {
    let len = f.len();
    if len == 0 {
        "[]".to_string()
    } else if len <= 2 {
