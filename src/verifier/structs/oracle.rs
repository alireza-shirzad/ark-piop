...
/// Backward-compatible alias: the verifier uses the same [`VirtualPoly`] type
/// as the prover to represent symbolic sum-of-products expressions.
pub type VirtualOracle<F> = VirtualPoly<F>;
...
pub(crate) enum InnerOracle<F: Field + 'static> {
...
impl<F: 'static + Field> Oracle<F> {
    ...
    pub(crate) fn inner(&self) -> &InnerOracle<F> {
        &self.inner
    }
}
...
impl<F: PrimeField> Oracle<F> {
    ...
    pub fn from_mle(poly: crate::arithmetic::mat_poly::mle::MLE<F>) -> Self {
        ...
        Self::new_multivariate(nv, move |pt: Vec<F>| Ok(poly.evaluate(&pt)))
    }
}
    ...
    B: SnarkBackend,
{
    ...
    /// Cached tracker ID for constant oracles. Set when this is a committed
    /// constant, or lazily populated on first `.id()` call for derived constants.
    ...
    B: SnarkBackend,
{
    ...
    /// Create a new tracked oracle for a committed constant.
    ...
    pub fn id(&self) -> TrackerID {
        match &self.id_or_const {
            ...
            Either::Right(cnst) => {
                ...
                // Derived constant — materialize as a constant oracle so
                // claims can reference it. The prover mirrors this call.
                let oracle = Oracle::new_constant(self.log_size, *cnst);
                let id = self.tracker.borrow_mut().track_base_oracle(oracle);
    ...
    /// `true` if this tracked oracle is a folded constant.
    ...
    pub(crate) fn assert_same_tracker(&self, other: &TrackedOracle<B>) {
        assert!(
...
impl<'b, B: SnarkBackend> Add<&'b TrackedOracle<B>> for &TrackedOracle<B> {
...
impl<'b, B: SnarkBackend> Sub<&'b TrackedOracle<B>> for &TrackedOracle<B> {
    type Output = TrackedOracle<B>;
...
impl<'b, B: SnarkBackend> Mul<&'b TrackedOracle<B>> for &TrackedOracle<B> {
    type Output = TrackedOracle<B>;
    ...
    B::F: PrimeField,
{
    ...
    match verifier.indexed_tracked_poly(label.clone()) {
        Ok(oracle) => oracle,
        Err(_) => {
            let oracle = build_shift_oracle::<B::F>(log_size, shift, is_right);
            let oracle = verifier.track_base_oracle(oracle);
