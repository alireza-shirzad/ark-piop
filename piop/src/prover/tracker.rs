    ...
    UvPCS: PCS<F, Poly = LDE<F>>,
{
    ...
    pub fn track_mat_mv_poly(&mut self, polynomial: MLE<F>) -> TrackerID {
        ...
        let output = self.track_mat_arc_mv_poly(polynomial);
        add_trace!("track_mat_mv_poly", "id = {:?}", output.0);
        output
    }
    ...
    pub fn track_mat_uv_poly(&mut self, polynomial: LDE<F>) -> TrackerID {
        let polynomial = Arc::new(polynomial);
        let output = self.track_mat_arc_uv_poly(polynomial);
        add_trace!("track_mat_uv_poly", "id = {:?}", output.0);
        output
    }
    ...
    fn track_mat_arc_mv_poly(&mut self, polynomial: Arc<MLE<F>>) -> TrackerID {
        ...
        add_trace!("track_mat_arc_mv_poly", "id = {:?}", poly_id.0);
    ...
    pub fn track_and_commit_mat_mv_p(&mut self, polynomial: &MLE<F>) -> SnarkResult<TrackerID> {
        ...
        add_trace!("track_and_commit_mat_mv_p", "id = {:?}", poly_id.0);
        // Return the new TrackerID
        Ok(poly_id)
    }
    ...
    pub fn track_and_commit_mat_uv_poly(&mut self, polynomial: LDE<F>) -> SnarkResult<TrackerID> {
        ...
        self.state
            .transcript
            .append_serializable_element(b"comm", &commitment)?;
        add_trace!("track_and_commit_mat_uv_poly", "id = {:?}", poly_id.0);
        // Return the new TrackerID
        Ok(poly_id)
    }
    ...
    pub fn add_sub_polys(&mut self, p1_id: TrackerID, p2_id: TrackerID, do_sub: bool) -> TrackerID {
        ...
        let p1_mat = self.get_mat_mv_poly(p1_id);
        let p1_virt = self.get_virt_poly(p1_id);
        let p2_mat = self.get_mat_mv_poly(p2_id);
    ...
    pub fn mul_scalar(&mut self, poly_id: TrackerID, c: F) -> TrackerID {
        ...
        let p_mat = self.get_mat_mv_poly(poly_id);
        if p_mat.is_some() {
    ...
    pub(crate) fn to_hp_virtual_poly(&self, id: TrackerID) -> HPVirtualPolynomial<F> {
        ...
        for (prod_coef, prod) in poly.iter() {
            let prod_mle_list = prod
                .iter()
                .map(|poly_id| self.get_mat_mv_poly(*poly_id).unwrap().clone())
    ...
    fn equalize_mat_poly_nv(&mut self) -> usize {
        ...
        let true_sums: Vec<F> = old_sumcheck_claims
            .iter()
            .map(|claim| self.evaluations(claim.get_id()).iter().sum::<F>())
            .collect();
    ...
    fn batch_s_check_claims(&mut self, max_nv: usize) -> SnarkResult<BTreeMap<TrackerID, F>> {
        ...
        let individual_sumcheck_claims: BTreeMap<TrackerID, F> = self
            ...
            .sum_check_claims
            .iter()
            .map(|claim| (claim.get_id(), claim.get_claim()))
            .collect();
    ...
    fn compile_sc_subproof(&mut self, max_nv: usize) -> SnarkResult<SumcheckSubproof<F>> {
        ...
        let individual_sumcheck_claims = self.batch_s_check_claims(max_nv)?;
